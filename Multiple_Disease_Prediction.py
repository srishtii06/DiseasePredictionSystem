...
def main():
    ...
    if( selected == 'Diabetes Prediction'):
        ...
        Pregnancies = st.number_input("Number of Pregnancies", format="%.0f", value=float(default_values["Pregnancies"]),step=1.0)
        Glucose = st.number_input("Glucose Level", format="%.2f", value=float(default_values["Glucose"]))
        BloodPressure = st.number_input("BloodPressure volume", format="%.2f", value=float(default_values["BloodPressure"]))
        SkinThickness = st.number_input("SkinThickness value", format="%.2f", value=float(default_values["SkinThickness"]))
        Insulin = st.number_input("Insulin level", format="%.2f", value=float(default_values["Insulin"]))
        BMI = st.number_input("BMI value", format="%.2f", value=float(default_values["BMI"]))
        DiabetesPedigreeFunction = st.number_input("DiabetesPedigreeFunction value", format="%.3f", value=float(default_values["DiabetesPedigreeFunction"]))
        Age = st.number_input("Age of the person", format="%.0f", value=float(default_values["Age"]))

        # Show message about default values being used
        if st.button("Use Default Values"):
            st.write("Default values are being used for the fields.")
    ...
    if( selected == 'Heart Disease Prediction'):
        ...
        with col3:
            option2 = st.selectbox('Chest Pain type',('0- Normal Pain','1- Unsual Chest Pain','2- Pain but not from the hear side','3 - No Pain'))
            if option2 == '0- Normal Pain':
                chest_pain = 0
            elif option2 == '1- Unsual Chest Pain':
                chest_pain = 1
            elif option2 == '2- Pain but not from the hear side':
                chest_pain = 2
            else:
                chest_pain = 3
        with col1:
            resting_bp = st.number_input("Resting Blood Pressure (in mm Hg)", value=120)
        with col2:
            serum_cholestoral = st.number_input("Serum Cholestoral in mg/dl", value=200)
        with col3:
            option3 = st.selectbox('Fasting Blood Sugar',('True (>120)', 'False (<=120)')) 
            fasting_blood_sugar = 0 if option3 == 'False' else 1
        with col1:
            option4 = st.selectbox('Resting ECG Results',('0-normal','1- abnormality (problem) in the heart’s electrical activity','2-bigger problem with the heart’s electrical activity'))
            if option4 == '0-normal':
                resting_ecg = 0
            elif option4 == '1- abnormality (problem) in the heart’s electrical activity':
                resting_ecg = 1
            elif option4 == '2-bigger problem with the heart’s electrical activity':
                resting_ecg = 2
        with col2:
            max_heart_achieved = st.number_input("Maximum Heart Rate Achieved", value=150)
        with col3:
        ...
        with col2:
            option6 = st.selectbox('The slope of the peak exercise ST segment',('0-Upsloping','1- Stays Level','2- Downsloping'))
            if option6 == '0-Upsloping':
                slope_of_peak_exercise = 0
            elif option6 == '1- Stays Level':
                slope_of_peak_exercise = 1
            elif option6 == '2- Downsloping':
                slope_of_peak_exercise = 2
        with col3:
            option7 = st.selectbox('The no. of major blood vessels that are blocked or narrwoed',('0','1','2','3','4'))
            if option7 == '0':
    ...
    if( selected == 'Breast Cancer Prediction'):
        ...
        # with col1:
        #     mean_radius = st.number_input("mean radius",value=14.0, format="%.6f")
        # with col2:
        #     mean_texture = st.number_input("mean texture", value=19.0,format="%.6f")
        # with col3:
        #     mean_perimeter = st.number_input("mean_perimeter", value=91.0,format="%.6f")
        # with col1:
        #     mean_area = st.number_input("mean_area",value=600.0,format="%.6f")
        # with col2:
        #     mean_smoothness = st.number_input("mean_smoothness", value=0.1, format="%.6f")
        # with col3:
        #     mean_compactness = st.number_input("mean_compactness", value=0.2,format="%.6f")
        # with col1:
        #     mean_concavity = st.number_input("mean_concavity",value=0.1,format="%.6f")
        # with col2:
        #     mean_concave_points = st.number_input("mean_concavepoints", value=0.05,format="%.6f")
        # with col3:
        #     mean_symmetry = st.number_input("mean_symmetry",value=0.2,format="%.6f")
        # with col1:
        #     mean_fractal_dimension = st.number_input("mean_fractal_dim",value=0.06,format="%.6f")
        # with col2:
        #     radius_error = st.number_input("radius_error",value=14.0,format="%.6f")
        # with col3:
        #     texture_error  = st.number_input("texture_error",value=19.0,format="%.6f")
        # with col1:
        ...
        with col2:
            mean_texture = st.number_input("mean texture", value=19.0, format="%.6f")
        with col3:
            mean_perimeter = st.number_input("mean perimeter", value=91.0, format="%.6f")
        with col1:
            mean_area = st.number_input("mean area", value=600.0, format="%.6f")
        with col2:
            mean_smoothness = st.number_input("mean smoothness", value=0.1, format="%.6f")
        with col3:
            mean_compactness = st.number_input("mean compactness", value=0.2, format="%.6f")
        with col1:
            mean_concavity = st.number_input("mean concavity", value=0.1, format="%.6f")
        with col2:
            mean_concave_points = st.number_input("mean concave points", value=0.05, format="%.6f")
        with col3:
            mean_symmetry = st.number_input("mean symmetry", value=0.2, format="%.6f")
        with col1:
            mean_fractal_dimension = st.number_input("mean fractal dimension", value=0.06, format="%.6f")
        with col2:
            radius_error = st.number_input("radius error", value=0.3, format="%.6f")
        with col3:
            texture_error = st.number_input("texture error", value=1.0, format="%.6f")
        with col1:
            perimeter_error = st.number_input("perimeter error", value=2.0, format="%.6f")
        with col2:
            area_error = st.number_input("area error", value=25.0, format="%.6f")
        with col3:
            smoothness_error = st.number_input("smoothness error", value=0.005, format="%.6f")
        with col1:
            compactness_error = st.number_input("compactness error", value=0.02, format="%.6f")
        with col2:
            concavity_error = st.number_input("concavity error", value=0.02, format="%.6f")
        with col3:
            concave_points_error = st.number_input("concave points error", value=0.01, format="%.6f")
        with col1:
            symmetry_error = st.number_input("symmetry error", value=0.02, format="%.6f")
        with col2:
            fractal_dimension_error = st.number_input("fractal dimension error", value=0.003, format="%.6f")
        with col3:
            worst_radius = st.number_input("worst radius", value=16.0, format="%.6f")
        with col1:
            worst_texture = st.number_input("worst texture", value=25.0, format="%.6f")
        with col2:
            worst_perimeter = st.number_input("worst perimeter", value=107.0, format="%.6f")
        with col3:
            worst_area = st.number_input("worst area", value=880.0, format="%.6f")
        with col1:
            worst_smoothness = st.number_input("worst smoothness", value=0.15, format="%.6f")
        with col2:
            worst_compactness = st.number_input("worst compactness", value=0.3, format="%.6f")
        with col3:
            worst_concavity = st.number_input("worst concavity", value=0.2, format="%.6f")
        with col1:
            worst_concave_points = st.number_input("worst concave points", value=0.1, format="%.6f")
        with col2:
            worst_symmetry = st.number_input("worst symmetry", value=0.3, format="%.6f")
        with col3:
            worst_fractal_dimension = st.number_input("worst fractal dimension", value=0.08, format="%.6f")
